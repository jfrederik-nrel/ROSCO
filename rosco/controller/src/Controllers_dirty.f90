...
!-------------------------------------------------------------------------------------------------------------------------------  
    SUBROUTINE VariableSpeedControl(avrSWAP, CntrPar, LocalVar, DebugVar, PerfData, objInst, ErrVar)
    ...
    !       VS_State = 6, Tip-Speed-Ratio tracking PI controller
        ...
        IF ((CntrPar%VS_ControlMode == 2) .OR. (CntrPar%VS_ControlMode == 3)) THEN
            ...
            ELSE
                ...
                DebugVar%axisYaw_1P  = LocalVar%PulseGenTq
                DebugVar%axisTilt_2P = LocalVar%VS_SpdErr
...
!-------------------------------------------------------------------------------------------------------------------------------
    SUBROUTINE ActiveWakeControl(CntrPar, LocalVar, DebugVar, PerfData, objInst, ErrVar)
                ...
                DO Imode = 1,CntrPar%AWC_NumModes
                    ...
                    IF (CntrPar%AWC_Mode == 4) THEN
                        AWC_TiltYaw(Imode) = ResController(Error(Imode), CntrPar%AWC_CntrGains(1), CntrPar%AWC_CntrGains(2), CntrPar%AWC_freq(Imode), & 
                                                            CntrPar%PC_MinPit, CntrPar%PC_MaxPit, LocalVar%DT, AWC_TiltYaw(Imode), LocalVar%resP, LocalVar%restart, objInst%instRes)
                    ELSE
        ...
        ELSEIF (CntrPar%AWC_Mode == 6) THEN
            ...
            ! DebugVar%axisYaw_2P = NotchFilter(LocalVar%WE%v_h, LocalVar%DT, 2*PI*CntrPar%AWC_freq(1), 0.0, 0.8, &
            !                                 LocalVar%FP,LocalVar%iStatus,LocalVar%restart,objInst%instNotch, 0.0_DbKi) !AWC_TiltYaw(1) !LocalVar%GenTq
            ! DebugVar%axisTilt_2P = AWC_TiltYaw(1)!interp2d(PerfData%Beta_vec,PerfData%TSR_vec,PerfData%Ct_mat, &
            !                         !        LocalVar%BlPitchCMeas*R2D, lambda , ErrVar)
            ! DebugVar%axisYaw_1P = lambda !CntrPar%YawMean - CntrPar%AWC_amp(2)*sin(LocalVar%Time*2*PI*CntrPar%AWC_freq(2) + CntrPar%AWC_clockangle(1)*D2R)
            ! DebugVar%axisTilt_1P = LocalVar%GenTq !LocalVar%PulseGenTq

            ! WIP pulse closed-loop with pre-set TSR
        ELSEIF (CntrPar%AWC_Mode == 7) THEN

            ! Implement open-loop gen torque variations
            Error(2) = LocalVar%GenTq + CntrPar%AWC_amp(1)*sin(LocalVar%Time*2*PI*CntrPar%AWC_freq(1))
            LocalVar%PulseGenTq = ResController(Error(2), CntrPar%AWC_clockangle(1), CntrPar%AWC_clockangle(2), CntrPar%AWC_freq(1), & 
                                                                -1e10, 1e10, LocalVar%DT, LocalVar%VS_LastGenTrq, LocalVar%resP, LocalVar%restart, objInst%instRes)
            ...
            IF (LocalVar%Time .GT. 0) THEN !1/CntrPar%AWC_freq(1)) THEN
                ...
                Error(1) = interp2d(PerfData%Beta_vec,PerfData%TSR_vec,PerfData%Ct_mat, &
                                                LocalVar%BlPitchCMeas*R2D, lambda , ErrVar) & ! This is the CT estimator using look-up table
                                                !- 0.763 & ! This is my mean CT estimate. Perhaps this can be removed altogether?
                                + CntrPar%AWC_amp(2)*sin(LocalVar%Time*2*PI*CntrPar%AWC_freq(2)) ! This is the excitation as defined in the input
                
                ! Resonance controller similar to above
                AWC_TiltYaw(1) = ResController(Error(1), CntrPar%AWC_CntrGains(1), CntrPar%AWC_CntrGains(2), CntrPar%AWC_freq(1), & 
                                                                -1e10, 1e10, LocalVar%DT, AWC_TiltYaw(1), LocalVar%resP, LocalVar%restart, objInst%instRes)
